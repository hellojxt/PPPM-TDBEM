...
class GArr3D
{
  public:
    ...
    GPU_FUNC inline T &operator()(const int3 index) { return data[index.x * rows * cols + index.y * cols + index.z]; }
...
class CArr3D
{
  public:
    ...
    inline T &operator()(const uint b_i, const uint i, const uint j) { return data[b_i * rows * cols + i * cols + j]; }
