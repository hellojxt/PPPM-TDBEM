...
namespace pppm
{
    ...
    {
        return (ind1.x == ind2.x) + (ind1.x == ind2.y) + (ind1.x == ind2.z) +
               (ind1.y == ind2.x) + (ind1.y == ind2.y) + (ind1.y == ind2.z) +
               (ind1.z == ind2.x) + (ind1.z == ind2.y) + (ind1.z == ind2.z);
    }
    ...
    inline CGPU_FUNC cpx singular_point(float xsi, float eta1, float eta2, float eta3, float weight,
                                        float3 *trial_v, float3 *test_v, float3 trial_norm, int neighbor_num, cpx s, PotentialType type)
    {
        cpx result = cpx(0, 0);
        xsi = 0.5 * (xsi + 1);
        eta1 = 0.5 * (eta1 + 1);
    ...
    inline CGPU_FUNC cpx singular_integrand(float3 *trial_v, float3 *test_v,
                                            float trial_jacobian, float test_jacobian, cpx s, int neighbor_num, PotentialType type)
    {
    ...
    inline CGPU_FUNC cpx regular_integrand(float3 *trial_v, float3 *test_v,
                                           float trial_jacobian, float test_jacobian, cpx s, PotentialType type)
    {
        cpx result = cpx(0, 0);
        ...
        for (int i = 0; i < TRI_GAUSS_NUM; i++)
            for (int j = 0; j < TRI_GAUSS_NUM; j++)
            {
                ...
                result += 0.25 * guass_w[i] * guass_w[j] * trial_jacobian * test_jacobian * layer_potential(v1, v2, trial_norm, s, type);
            }
        return result;
    }

    inline CGPU_FUNC cpx potential_integrand(float3 point, float3 *src_v,
                                             float src_jacobian, cpx s, PotentialType type)
    {
        cpx result = cpx(0, 0);
        float guass_x[TRI_GAUSS_NUM][2] = TRI_GAUSS_XS;
        float guass_w[TRI_GAUSS_NUM] = TRI_GAUSS_WS;
    ...
    inline CGPU_FUNC cpx face2FaceIntegrand(float3 *vertices, int3 src, int3 trg, cpx k, PotentialType type)
    {
    ...
    inline CGPU_FUNC cpx face2PointIntegrand(float3 *vertices, int3 src, float3 trg, cpx k, PotentialType type)
    {
        float3 src_v[3] = {{vertices[src.x]}, {vertices[src.y]}, {vertices[src.z]}};
        float src_jacobian = jacobian(src_v);
