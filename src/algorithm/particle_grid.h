...
class ParticleGrid
{
  public:
    ...
    GArr<Range> grid_dense_map;   // grid_dense_map(i) is the index range (index for particles) of the elements in the
                                  // i-th non-empty grid cell. Range is [start, end) and grid is sorted by morton code
    GArr3D<Range> grid_hash_map;  // grid_hash_map(i,j,k) is the index range (index for particles) of the elements in
                                  // the grid cells. If the grid cell is empty, the range is [0,0).
