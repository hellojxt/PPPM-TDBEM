...
namespace pppm
{
    ...
    {
        out << " cell_coord: " << be.cell_coord
            << " pos: " << be.pos << " normal: " << be.normal << std::endl;
        return out;
    }
    ...
    __global__ void grid_dense_map_post_kernel(GArr<BElement> particles, GArr<Range> grid_dense_map, GArr3D<Range> grid_hash_map)
    {
        int idx = threadIdx.x + blockIdx.x * blockDim.x;
