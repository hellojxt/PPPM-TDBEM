...
#define STEP_NUM 64
    // number of time steps for history of each particle
    ...
    {
    public:
        cpx wave_numbers[STEP_NUM]; // wave number for transforming from lapalace domain to time domain and vice versa
    ...
    CGPU_FUNC inline void TDBEM::scaledDFT(cpx *in, cpx *out)
    {
        for (int k = 0; k < STEP_NUM; k++)
        {
            out[k] = 0;
    ...
    CGPU_FUNC inline void TDBEM::laplace_weight(float3 *vertices, PairInfo pair, PotentialType potential_type, cpx *weight)
    {
        cpx v[STEP_NUM];
        for (int k = 0; k < STEP_NUM; k++)
        {
    ...
    CGPU_FUNC inline cpx TDBEM::helmholtz(float3 *vertices, PairInfo pair, cpx src_neumann, cpx src_dirichlet, cpx wave_number)
    {
