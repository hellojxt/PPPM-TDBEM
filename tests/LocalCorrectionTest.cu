...
TEST_CASE("Local Correction", "[lc]")
{

#define LC_TEST_MAX_STEP 256
    ...
    float3 vertices[3] = {
        center + make_float3(0.4f, 0.0f, 0.0f) * fdtd.dl,
    ...
    float check_fdtd[LC_TEST_MAX_STEP];
    float check_bem[LC_TEST_MAX_STEP];
    int3 check_coord = make_int3(30, 30, 30);

    while (fdtd.t < LC_TEST_MAX_STEP)
    {
        int t = fdtd.t;
