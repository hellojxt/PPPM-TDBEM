...
class BEMModel():
    ...
    def __init__(self, vertices, elements, wave_number):
        '''
        vertices: (n, 3) array
        elements: (m, 3) array
        '''
    ...
    def boundary_equation_solve(self, neumann_coeff):
        ...
        M = boundary.sparse.identity(
            self.dp0_space, self.dp0_space, self.dp0_space, precision="single", device_interface="opencl")
        K = boundary.helmholtz.double_layer(
            self.dp0_space, self.dp0_space, self.dp0_space, self.k, precision="single", device_interface="opencl")
    ...
    def potential_solve(self, points):
        ...
        potential_single = potential.helmholtz.single_layer(
            self.dp0_space, points.T, self.k, precision="single", device_interface="opencl")
